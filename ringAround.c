...
int main(int argc, char** argv) {
	...
	if (world_rank == 0) {
		...
		printf("Doubles,Raw Fastest,Raw Slowest,Raw Average,Bytes,Calculated 
		Fastest,Calculated Slowest,Calculated Average\n");
	}
	...
	for (size = 0; size < 12; size++) {
		...
		if (world_rank == 0) {
			// Initialize slowest and average
			slowest = raw[0];
			fastest = raw[0];
			average = 0;
			...
			// Print 
			printf("%d,%f,%f,%f,%d,%f,%f,%f\n", SIZES[size], fastest, slowest, 
			average, 8*SIZES[size], fastest/16, slowest/16, average/16);
		}
	}
